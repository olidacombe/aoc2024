...
impl Region {
    fn perimeter(&self) -> usize {
        self.edges.iter().filter(|(_, count)| *count == &1).count()
    }
...
mod test {
    ...
    fn tiny_full_example() -> Result<()> {
        let example = indoc! {"
            ...
            EEEC
        "};
        assert_eq!(fence_price(example.lines().map(String::from))?, 140);
        Ok(())
    }
    ...
    fn small_example() -> Result<()> {
        let example = indoc! {"
            ...
            OOOOO
        "};
        assert_eq!(fence_price(example.lines().map(String::from))?, 772);
        Ok(())
    }
    ...
    fn full_example() -> Result<()> {
        let example = indoc! {"
            ...
            MMMISSJEEE
        "};
        assert_eq!(fence_price(example.lines().map(String::from))?, 1930);
        Ok(())
    }
}
